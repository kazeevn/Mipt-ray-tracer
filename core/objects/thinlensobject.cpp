...
ThinLensObject::ThinLensObject(const Point3D& point, const Vector3D& v1, const Vector3D& v2, const QImage& bitmask, double focus)
    : RectanglePolygon(point, v1, v2),
      ...
      m_frontFocalPlane(opticalCentre+m_perpendicular*fabs(m_focus), m_perpendicular),
      m_backFocalPlane(opticalCentre+m_perpendicular*(-fabs(m_focus)), m_perpendicular)
{
}
...
{
    Point3D *p = 0;
    Ray3D helperRay(opticalCentre, ray.direction() * (m_focus > 0 ? 1 : (-1)));
